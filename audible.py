...
class AudiblePage:
    ...
    def __init__(self, category_name, category_url, page_number, web):
        ...
        self.html_list_of_items = self.get_html_list_of_items(
            web, settings.BOOK_ITEM_SELECT
        )
        self.items = self.getAudibleItems(
            settings.IMAGE_DIV_SELECT, settings.TEXT_DIV_SELECT
        )
    ...
    def getAudibleItems(self, IMAGE_DIV_SELECT, TEXT_DIV_SELECT) -> list:
        ...
        for html_for_item in self.html_list_of_items:
            new_item = AudibleItem(
                self.category_name, html_for_item, IMAGE_DIV_SELECT, TEXT_DIV_SELECT
            )
            if any(
                item.title == new_item.title and item.author == new_item.author
                for item in audible_items
            ):
                existing_item = next(
                    item
                    for item in audible_items
                    if item.title == new_item.title and item.author == new_item.author
                )
                existing_item.category_name = (
                    existing_item.category_name + ", " + self.category_name
                )
            else:
    ...
    def __eq__(self, other):
        if isinstance(other, AudiblePage) and self.html_list_of_items is not None:
            return (
                self.items[0].title == other.items[0].title
                and self.items[0].author == other.items[0].author
            )
...
class AudibleItem:
    ...
    def get_audible_link(self):
        ...
        audible_link = "https://www.audible.com" + self.image_div.find_all("a")[0].get(
            "href"
        )
        return audible_link
    ...
    def get_image_link(self):
        ...
        image_link = (
            self.image_div.find_all("img")[0]
            .get("src")
            .replace("_SL32_QL50_ML2_", "_SL500_")
        )
        return image_link
...
class Book:
    ...
    def already_in_df(self, df):
        if (
            df["Audible_Title"].str.contains(self.title, regex=False).any()
        ):  # If the title matches a title of a row in the df, check whether the author matches the author of that row in the df
            row_in_df = df["Audible_Title"].str.contains(self.title, regex=False)
            return (
                df.loc[row_in_df]["Audible_Author"]
                .str.contains(self.author, regex=False)
                .any()
            )  # Previously had .any() - maybe it needs that?
        else:
            return False
    ...
    def add_category_to_existing(self, category_name, df):
        ...
        if self.already_in_df(df):
            row_in_df = df.index[
                df["Audible_Title"].str.contains(self.title, regex=False)
            ]
            # print(df.loc[row_in_df, 'Audible_Category'].str.contains(category_name).any())
            if (
                not df.loc[row_in_df, "Audible_Category"]
    ...
    def is_in_df(self, df):
        if (
            (df["Audible_Title"] == self.title) & (df["Audible_Author"] == self.author)
        ).any():  # Get from df, then goodreads
            return True
        return False
