...
def getAudibleData(url, category, max_pages, df):
    page_number = 1
...
def getGoodreadsMatches(audible_df):
    possibles = pd.DataFrame({'Book ID':pd.Series([], dtype='int'),'Goodreads Title':[], 'Goodreads Author':[], 'Goodreads Link':[], 'Goodreads Search URL':[], 'Goodreads Rating':pd.Series([], dtype='float'), 'Number of Ratings':pd.Series([], dtype='int')})
    for index, row in audible_df.iterrows():
        title = row['Audible Title']
        author =  row['Audible Author'].replace('The Great Courses', '')
        search_url = 'https://www.goodreads.com/search?q=' + title.replace(' ', '+').replace("'","-") + '+' + author.replace(' ', '+').replace('PhD', '').replace('MD', '').replace('Dr', '')
        #print(url)
                ...
                for link in authorLinks:
                    ...
                    if len(re.sub(r'[^\w\s]','',author.replace("'", "")).split()) > 1:
                        if re.sub(r'[^\w\s]','',author.replace("'", "")).split()[1] in re.sub(r'[^\w\s]','',GRauthor.replace("'", "")).split() and re.sub(r'[^\w\s]','',GRtitle.replace("'", "")).split()[0] == re.sub(r'[^\w\s]','',title.replace("'", "")).split()[0]:
                            ...
                            possibleRow = [index, GRtitle, GRauthor, GRlink, search_url, float(GRrating), int(GRnumratings.replace(',', ''))]
                        ...
                        if re.sub(r'[^\w\s]','',GRtitle.replace("'", "")).split()[0] == re.sub(r'[^\w\s]','',title.replace("'", "")).split()[0]:
                            # print('GR Title matches: ' + GRtitle)
                            # print('Set rating')
                            GRrating, GRnumratings = getRating(row)
                            possibleRow = [index, GRtitle, GRauthor, GRlink, search_url, float(GRrating), int(GRnumratings.replace(',', ''))]
...
for _, (category, audibleurl) in enumerate(categories.items()):
    df = getAudibleData(audibleurl, category, max_pages, df)
