...
class Setup:
            ...
            for link in links:
                if link.text != "All Categories":
                    categories.append(
                        audible.Category(
                            link.text,
                            "https://www.audible.com" + link.get("href").split("&")[0],
                        )
                    )
        return categories
    ...
    def createBook(self, audibleItem):
        return audible.Book(
            audibleItem.title,
            audibleItem.author,
            audibleItem.audible_link,
            audibleItem.image_link,
            audibleItem.category_name,
            subtitle=audibleItem.subtitle,
        )
    ...
    def loadCategories(self, source="bin"):
        if source == "bin":
            ...
            categories = self.getCategories(
                web, settings.CATEGORY_SELECT, settings.BASE_URL
            )
        return categories
